...
class RobotController:
    ...
    def __init__(self):
        ...
        # A wide horizontal yellow section counts as a sharp turn only below
        # this fraction of the frame height. At 480p, 0.85 starts at row 408.
        self.horizontal_turn_zone_top = 0.85
        # With no learned road width, one visible border still produces a
        # gentle target 10% of the frame toward the missing side.
        self.single_line_target_shift = 0.10
    ...
    def detect_road(self, frame):
        ...
        horizontal_turn_y = int(h * self.horizontal_turn_zone_top)
        ...
        return {
            ...
            "horizontal_turn_y": horizontal_turn_y,
        }
    ...
    def trace_component(
        ...
        for y in y_values:
            ...
            if run_width >= horizontal_minimum:
                # A horizontal section higher in the image is still far from
                # the car. Stop look-ahead here instead of following its far
                # endpoint and steering into the sharp turn prematurely.
                if y < horizontal_turn_y:
                    break
    ...
    def horizontal_turn_hint(
        ...
        for path in (left_path, right_path):
            ...
            directed_segments = [
                ...
                if (
                    segment["direction"] != 0
                    and segment["y"] >= horizontal_turn_y
                )
            ]
    ...
    def calculate_navigation(self, detection, image_width, image_height):
        ...
        left_path = detection["left_path"]
        right_path = detection["right_path"]
        ...
        if centers:
            ...
            near = centers[0]
            far = centers[-1]
            target_x = near["x"] + self.lookahead_weight * (far["x"] - near["x"])
            mode = far["mode"]
        elif (left_path is None) != (right_path is None):
            # A single connected border is not road loss. Aim a little toward
            # the missing side until the second border enters the camera:
            #
            #   left border visible  -> road continues to image-right
            #   right border visible -> road continues to image-left
            #
            # This fallback does not require a previously learned lane width.
            visible_path = left_path if left_path is not None else right_path
            missing_side_direction = 1 if left_path is not None else -1
            target_x = (
                image_width / 2.0
                + missing_side_direction
                * image_width
                * self.single_line_target_shift
            )
            near = {
                "x": visible_path["near_x"],
                "y": visible_path["bottom_y"],
            }
            far = near
            mode = (
                "left only fallback"
                if left_path is not None
                else "right only fallback"
            )
            ...
            near = {
                ...
                "y": int(image_height * lookahead_weight),
            }
            far = near
    ...
    def draw_detection(self, frame, detection):
        h, w = frame.shape[:2]
        horizontal_turn_y = detection["horizontal_turn_y"]
        ...
        # Magenta marks the top of the active sharp-turn zone. Only horizontal
        # yellow sections below this line can generate orange turn evidence.
        cv2.line(
            frame,
            (0, horizontal_turn_y),
            (w - 1, horizontal_turn_y),
            (255, 0, 255),
            2,
        )
        cv2.putText(
            frame,
            "SHARP TURN ZONE",
            (10, min(h - 8, horizontal_turn_y + 20)),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.50,
            (255, 0, 255),
            2,
    ...
    @staticmethod
    def draw_path(frame, path, color, horizontal_turn_y=0):
        if path is None:
            return
        ...
        for segment in path["horizontal_segments"]:
            if segment["y"] < horizontal_turn_y:
                continue
            cv2.line(
                frame,
